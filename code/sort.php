...
if (!$_GET['id']) header("Location:".$config['home_url']);

$id = htmlspecialchars(trim($_GET['id']));
$row = $db->getarray($db->query("SELECT * FROM ".$db_prefix."sort WHERE `name` = '$id' ORDER BY id DESC  LIMIT 0,1"));
$id=$row[id];
$st=$row[title];
//recent posts

	$row = $db->getarray($db->query("SELECT count(id) FROM ".$db_prefix."article WHERE `sort` = '$id'"));
	...
	if ($page>1)
		{
			$pagep.="<a href=\"".$config['home_url'].$tag."/\">首页</a>&nbsp;&nbsp;";
			$pagep.="<a href=\"".$config['home_url'].$tag."/".$prev."\">前一页</a>";
		}
	if ($page<$pages)
		{
			$pagen.="<a href=\"".$config['home_url'].$tag."/".$next."\">后一页</a>&nbsp;&nbsp;";
			$pagen.="<a href=\"".$config['home_url'].$tag."/".$last."\">末页</a>&nbsp;";
		}
	$t->set_var(array(
		"Info"=>"分类为 $st 的文章.",
		"Pre"=>$pagep,
		"Next"=>$pagen,
		"BlogPageTitle"=>$st."--".$config['sitename']));

	$query = "SELECT * FROM ".$db_prefix."article WHERE `sort` = '$id' ORDER BY date DESC  LIMIT $offset,$pagesize";
